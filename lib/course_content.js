...
function buildContentResponse(rawCourses, overrides) {
  ...
  if (hasLong && COURSE_CONTENT['long']) {
    content['long'] = Object.assign({}, COURSE_CONTENT['long'], ov['long'] && ov['long'].link ? { link: ov['long'].link } : {});
  }
  return content;
}
...
async function loadOverrides(supabase) {
  ...
  try {
    const { data } = await supabase.from('products').select('slug, drive_link, active');
    (data || []).forEach((p) => {
      if (p && p.drive_link && p.active !== false && (p.slug === 'short' || p.slug === 'long')) {
        out[p.slug] = { link: p.drive_link };
      }
    });
