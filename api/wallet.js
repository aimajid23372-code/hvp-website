...
module.exports = async (req, res) => {
  ...
  try {
    const { action, email, amount, course, promoCode, ref } = req.body;
    ...
    if (action === "topup") {
      ...
      const zpRes = await fetch("https://api.zinipay.com/v1/payment/create", {
        ...
        body: JSON.stringify({
          amount, cus_name: "Wallet User", cus_email: cleanEmail, cus_phone: "01000000000",
      ...
      if (!zpRes.ok || !zpData.payment_url) return res.status(502).json({ error: "Payment gateway error", details: zpData });
      
      let insertPayload = { customer_name: "Wallet Topup", customer_contact: cleanEmail, course: "wallet_topup", amount, invoice_id: extractInvoiceId(zpData), our_ref: ourRef, status: "pending" };
      let { error: insertErr } = await supabase.from("orders").insert(insertPayload);
      if (insertErr && insertErr.code === "PGRST204") {
        delete insertPayload.invoice_id; delete insertPayload.our_ref;
    ...
    if (action === "pay") {
      ...
      if (promoCode) {
        ...
        if (promo) finalAmount = Math.round(finalAmount - (finalAmount * promo.discount_percent) / 100);
      }
      ...
      if (balance < finalAmount) return res.status(400).json({ error: "Insufficient wallet balance" });

      const newBal = balance - finalAmount;
      await supabase.from("wallets").update({ balance: newBal }).eq("email", cleanEmail);

      const ourRef = crypto.randomUUID();
      let insertPayload = { customer_name: "Wallet User", customer_contact: cleanEmail, course, amount: finalAmount, promo_code: promoCode || null, affiliate_ref: ref || null, invoice_id: "wallet_" + Date.now(), our_ref: ourRef, status: "paid", payment_method: "wallet" };
      let { error: insertErr } = await supabase.from("orders").insert(insertPayload);
      if (insertErr && insertErr.code === "PGRST204") {
        delete insertPayload.promo_code; delete insertPayload.affiliate_ref;
        const retry = await supabase.from("orders").insert(insertPayload);
        insertErr = retry.error;
      }
      if (insertErr) {
        // Rollback wallet if db fails
        await supabase.from("wallets").update({ balance }).eq("email", cleanEmail);
